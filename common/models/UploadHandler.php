...
class UploadHandler {
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if ($name === null){
            $a = pathinfo($this->getName(), PATHINFO_EXTENSION);
            $name = substr(preg_replace('/[^\da-z]/i', '', $this->getName()), 0,-3).'.'.$a;
        ...
        # chunked upload
            ...
            if (!file_exists($targetFolder)){
                ...
                mkdir($targetFolder.DIRECTORY_SEPARATOR.'thumbnail', 0777, true);
            }
            $target = $targetFolder.'/'.$partIndex;
            $targetThumb = $targetFolder.'/thumbnail/'.$partIndex;
            $success = move_uploaded_file($_FILES[$this->inputName]['tmp_name'], $target);
            $this->createThumbnail($target, $targetThumb, 160);
            return array("success" => true, "uuid" => $uuid);
        }
        else {
        # non-chunked upload
            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, $name));
            $targetThumb = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, 'thumbnail' , $name));
            if ($target){
    ...
    public function createThumbnail($src, $dest, $targetWidth, $targetHeight = null) {
        ...
        if (!$type || !$this->imagehandler[$type]) {
            return null;
        }

        $image = call_user_func($this->imagehandler[$type]['load'], $src);
        ...
        return call_user_func(
            $this->imagehandler[$type]['save'],
            $thumbnail,
            $dest,
            $this->imagehandler[$type]['quality']
        );
    }
