...
(function(undefined) {
    ...
    var args = {
        ...
        resultFunction: function(res) {
            ...
            $.ajax({
                type: 'POST',
                url: "https://mifg.ifg-life.id/verify-agent, 
                data: {data : res.code},
                dataType:"json",
                success: function(resultData) {
                    if(resultData.respcode == '1'){
                        document.location.href='/form-permintaan/'+resultData.agen;
                    }else{
                        alert("Maaf agent tidak di temukan");
                    }
                }
            });

            /*
            $.getJSON( "https://mifg.ifg-life.id/verify-agent/"+res.code, function( data ) {
                  if(resp.data != null){
                    document.location.href='/form-permintaan/'+res.code;
                  }else{
                    alert("Maaf agent tidak di temukan");
                  }
            });
