...
class SiteController extends Controller
{
    ...
    public function actionIndex($slug="", $agent="")
    {
        ...
        switch ($slug) {
            case 'form-permintaan':
				...
				$getSecret = $this->getSecretManager();
				var_dump($getSecret);
			...
			case 'form-permintaan-smpl':
				...
				$curl = curl_init();
						...
						curl_setopt_array($curl, array(
						  CURLOPT_URL => 'https://asuransi.ifg-life.id/api/mifg/?noagen=9999999999',
						  CURLOPT_RETURNTRANSFER => true,
						  CURLOPT_ENCODING => '',
						  CURLOPT_MAXREDIRS => 10,
						  CURLOPT_TIMEOUT => 0,
				...
				*/
				...
				$kirimdata=[
					'applicationList' => [
						...
						curl_setopt_array($curl, array(
						  CURLOPT_URL => 'http://10.10.1.150/mi/individual_insurance',
						  CURLOPT_RETURNTRANSFER => true,
						  //CURLOPT_ENCODING => '',
						  //CURLOPT_MAXREDIRS => 10,
						  CURLOPT_TIMEOUT => 0,
						...
						if($res["resp"]['resultCode']=='1000'){
							...
							curl_setopt_array($curl, array(
							  CURLOPT_URL => 'http://10.10.1.150/mi/upload_spaj',
							  CURLOPT_RETURNTRANSFER => true,
							  CURLOPT_ENCODING => '',
							  CURLOPT_MAXREDIRS => 10,
							  CURLOPT_TIMEOUT => 0,
    ...
    public function actionCheckAgent($id){
		...
		$response = $curl->setRawPostData(\yii\helpers\Json::encode($params))
		    ...
		    ->setOption(CURLOPT_SSL_VERIFYHOST, '0')
		    ->setOption(CURLOPT_SSL_VERIFYPEER, '0')
		    ->post('http://10.10.1.150/ifg/agen');
    ...
    public function getAddress(){
		...
		$response = $curl->setRawPostData(\yii\helpers\Json::encode($params))
		    ...
		    ->setOption(CURLOPT_SSL_VERIFYHOST, '0')
		    ->setOption(CURLOPT_SSL_VERIFYPEER, '0')
		    ->post('http://10.10.1.150/address/mapping');
	...
	function push_notif_email($nomorSPAJ){
		...
		$response = $curl->setRawPostData(\yii\helpers\Json::encode($params))
		    ...
		    ->setOption(CURLOPT_SSL_VERIFYHOST, '0')
		    ->setOption(CURLOPT_SSL_VERIFYPEER, '0')
		    ->post('http://10.10.1.150/notification/email');
	...
	function push_notif_email_rejected($nomorSPAJ){
		...
		$response = $curl->setRawPostData(\yii\helpers\Json::encode($params))
		    ...
		    ->setOption(CURLOPT_SSL_VERIFYHOST, '0')
		    ->setOption(CURLOPT_SSL_VERIFYPEER, '0')
		    ->post('http://10.10.1.150/notification/email_reject');
	...
	public function actionClosingSave(){
		...
		// if(!$model->save()){
			...
			$path = "closing/";
				...
				curl_setopt_array($curl, array(
				  CURLOPT_URL => 'http://10.10.1.150/mi/upload_closing_agent',
				  CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => '',
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 0,
	...
	private function getSecretManager(){
		...
		$secretName = 'arn:aws:iam::391510675580:role/sm-ifgl-jg2mq-nprd-dev-legacy-mifg';
		...
		try {
			...
			if (isset($result['SecretString'])) {
				...
				return $secret = base64_decode($result['SecretBinary']);
			}
