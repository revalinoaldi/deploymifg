...
pipeline {
    ...
    stages {
                stage("security-fortify-scan") {
                    ...
                    environment {
                        fortify_app_name = 'mifgMidwareService'
                         ...
                         '''
                            script {
                                if (env.BRANCH_NAME == 'uat') {
                                    String result = nexusscan("ifgl-middleware", "$WORKSPACE", "stage-release");
                                    echo result;
                                } else if (env.BRANCH_NAME == 'release') {
                                    echo '=================OSS RELEASE, PLEASE ACCESS THE REPORT ON https://oss.ifg-life.id ================='
                                    String result = nexusscan("ifgl-middleware", "$WORKSPACE", "release");
                                    echo result;
                                }
                    }
                }
