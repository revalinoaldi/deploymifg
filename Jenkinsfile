...
pipeline {
    ...
    stages {
                stage("security-fortify-scan") {
                    ...
                    environment {
                        fortify_app_name = 'mifgTranslatorService'
                         ...
                         '''
                            script {
                                if (env.BRANCH_NAME == 'uat') {
                                    String result = nexusscan("ifgl-translator", "$WORKSPACE", "stage-release");
                                    echo result;
                                } else if (env.BRANCH_NAME == 'release') {
                                    echo '=================OSS RELEASE, PLEASE ACCESS THE REPORT ON https://oss.ifg-life.id ================='
                                    String result = nexusscan("ifgl-translator", "$WORKSPACE", "release");
                                    echo result;
                                }
                    }
                }
